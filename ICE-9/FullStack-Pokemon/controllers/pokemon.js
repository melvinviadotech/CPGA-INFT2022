...
async function editPokemonPage(req, res) {
  try {
    const myPokemon = await Pokemon.find({});
    res.render("editPokemon.ejs", { myPokemon });
  } catch (err) {
    console.err("Error with getting pokemon");
    res.status(500).send("Error in getting pokemon");
  }
}
